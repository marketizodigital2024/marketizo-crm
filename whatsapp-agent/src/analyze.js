...
const MESSAGE_PROMPT = [
  ...
  "The ownerReason must explain the concrete business impact. recommendedAction must assign operational work to the appropriate Marketizo team role, never to Miljan or Ivana. Do not tell Miljan or Ivana to collect materials, reply to clients, check campaigns, chase deadlines, coordinate staff, or perform any other operational task. If an actual owner-only decision is required, state only the exact decision needed and do not turn it into an operational assignment. Do not use generic advice.",

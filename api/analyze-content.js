...
export default async function handler(request, response) {
  ...
  const evidence = posts.map((post, index) => {
    ...
    return { index, platform: post.platform, username: post.username, format: post.video ? "reel_video" : "post", caption: clean(post.caption, 4000), image: clean(post.image, 1000), url: clean(post.url, 1000), metrics: { likes: post.likes ?? null, comments: post.comments ?? null, views: post.views ?? null }, transcript };
  });

  const input = [{ type: "input_text", text: `Ti si senior Marketizo strateg. Piši prirodnim, gramatički tačnim srpskim jezikom (ekavica), kao iskusan čovek, bez AI fraza. Analiziraš samo dokaze koje dobiješ. Ne pretpostavljaj da nešto nedostaje ako to nisi proverio. Ne menjaj korisnikove reči prostim ubacivanjem u šablon. Ne izmišljaj grad, trajanje, cenu, format, rezultat ili CTA. Razlikuj podatak iz upitnika od onoga što je javno pronađeno. Ako se prodaja završava pozivom, cena i svi detalji ne moraju biti javni. Svaki prioritet i svaka ocena moraju navesti konkretan dokaz. Primer prepravke mora biti smislen za konkretnu objavu, bez neprirodnih padeža.\n\nPODACI O BIZNISU:\n${JSON.stringify(form)}\n\nPREGLEDANI SADRŽAJ:\n${JSON.stringify(evidence.map(({ image, ...item }) => item))}` }];
  evidence.filter(item => item.image).slice(0, 10).forEach(item => {
    try {
      input.push({ type: "input_text", text: `Vizuelni dokaz za objavu #${item.index}:` });

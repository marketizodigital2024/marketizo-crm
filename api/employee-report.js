...
function canLead(leader, employee) {
  return Boolean(leader?.isLeader && employee && (isOwnerLeader(leader) || employee.leaderId === leader.id));
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (body.action === "sendWeeklyQuestionnaire") {
      ...
      if (!leaderId || !/^\d{4}-\d{2}-\d{2}$/.test(weekKey) || !/^\d{4}-\d{2}-\d{2}$/.test(dueDate)) {
        return json(res, 400, { ok: false, error: "Nedostaje lider ili nedelja upitnika." });
      }
      for (let attempt = 0; attempt < 5; attempt += 1) {
        ...
        if (!leader) return json(res, 403, { ok: false, error: "Samo aktivan lider može poslati upitnik." });
        const employees = (payload.employees || []).filter((item) => item.id !== leaderId && item.status !== "Neaktivan" && canLead(leader, item));
        payload.employeeReports = Array.isArray(payload.employeeReports) ? payload.employeeReports : [];
        const now = new Date().toISOString();
        const invitations = employees.map((employee) => {
          let report = payload.employeeReports.find((item) => item.isWeeklyQuestionnaire === true && item.employeeId === employee.id && item.weekKey === weekKey);
          if (!report) {
            report = {
              id: `weekly-report-${employee.id}-${weekKey}`,
              employeeId: employee.id,
              recipientId: leaderId,
              weekKey,
              date: dueDate,
              dueDate,
              title: "Nedeljni izveštaj za lidera",
              isWeeklyQuestionnaire: true,
              status: "sent",
              sentAt: now,
              createdAt: now,
            };
            ...
            Object.assign(report, { recipientId: leaderId, dueDate, date: dueDate, status: "sent", sentAt: now });
          }
          return report;
        });
        ...
        const response = await fetch(`${url}/rest/v1/${tableName}?id=eq.${encodeURIComponent(rowId)}&updated_at=eq.${encodeURIComponent(row.updated_at)}&select=updated_at`, {
          method: "PATCH", headers: headers(key, "return=representation"), body: JSON.stringify({ payload, updated_at: updatedAt }),
        });
        if (!response.ok) throw new Error(`Slanje upitnika nije uspelo (${response.status}).`);
        const updatedRows = await response.json();
        if (updatedRows.length) return json(res, 200, { ok: true, invitations, updatedAt });
      }
      return json(res, 409, { ok: false, error: "Baza je trenutno zauzeta drugim upisima. Pokušaj ponovo." });
    }
    if (body.action === "submitWeeklyQuestionnaire") {
      const employeeId = String(body.employeeId || "");
      const reportId = String(body.reportId || "");
      const answers = Array.isArray(body.answers) ? body.answers.map(cleanAnswer).slice(0, 5) : [];
      if (!employeeId || !reportId || answers.length !== 5 || answers.some((answer) => !answer)) {
        return json(res, 400, { ok: false, error: "Odgovori na svih pet pitanja su obavezni." });
      }
      for (let attempt = 0; attempt < 5; attempt += 1) {
        const row = await readRow(url, key);
        if (!row) return json(res, 404, { ok: false, error: "Glavni zapis baze nije pronađen." });
        ...
        const report = (payload.employeeReports || []).find((item) => item.id === reportId && item.employeeId === employeeId && item.isWeeklyQuestionnaire === true);
        if (!employee || !report) return json(res, 403, { ok: false, error: "Upitnik nije pronađen za ovog zaposlenog." });
        Object.assign(report, { answers, status: "completed", submittedAt: new Date().toISOString(), acknowledgedAt: null, acknowledgedBy: null });
        const updatedAt = new Date().toISOString();
        const response = await fetch(`${url}/rest/v1/${tableName}?id=eq.${encodeURIComponent(rowId)}&updated_at=eq.${encodeURIComponent(row.updated_at)}&select=updated_at`, {
          method: "PATCH", headers: headers(key, "return=representation"), body: JSON.stringify({ payload, updated_at: updatedAt }),
        });
        if (!response.ok) throw new Error(`Čuvanje nedeljnog izveštaja nije uspelo (${response.status}).`);
        const updatedRows = await response.json();
        if (updatedRows.length) return json(res, 200, { ok: true, report, updatedAt });
      }
      return json(res, 409, { ok: false, error: "Baza je trenutno zauzeta drugim upisima. Pokušaj ponovo." });
    }
    if (body.action === "acknowledgeWeeklyQuestionnaire") {
      const leaderId = String(body.leaderId || "");
      const reportId = String(body.reportId || "");
      for (let attempt = 0; attempt < 5; attempt += 1) {
        const row = await readRow(url, key);
        ...
        const payload = JSON.parse(JSON.stringify(row.payload || {}));
        const leader = (payload.employees || []).find((item) => item.id === leaderId && item.status !== "Neaktivan" && item.isLeader);
        const report = (payload.employeeReports || []).find((item) => item.id === reportId && item.isWeeklyQuestionnaire === true && item.submittedAt);
        const employee = (payload.employees || []).find((item) => item.id === report?.employeeId);
        if (!leader || !report || !canLead(leader, employee)) return json(res, 403, { ok: false, error: "Ovaj izveštaj nije dodeljen tom lideru." });
        ...
        if (!response.ok) throw new Error(`Potvrda nedeljnog izveštaja nije uspela (${response.status}).`);
        const updatedRows = await response.json();
        if (updatedRows.length) return json(res, 200, { ok: true, report, updatedAt });
      }
      return json(res, 409, { ok: false, error: "Baza je trenutno zauzeta drugim upisima. Pokušaj ponovo." });

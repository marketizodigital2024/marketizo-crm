...
function renderResults(){
  if(!data)return;const fc=$('filterClient').value,fe=$('filterEmployee').value,fr=$('filterRole').value,fm=$('filterMonth').value;
  ...
  const scores=[],teamScores=[];for(const r of rows)for(const q of r.questions){if(q.type!=='rating')continue;const n=Number(r.answers[q.id]);if(!(n>=1&&n<=5))continue;if(q.target==='team'){if(!fe&&!fr)teamScores.push(n);continue;}if(fr&&q.target!==fr)continue;if(fe&&q.targetEmployeeId!==fe)continue;scores.push(n);}
  const avg=values=>values.length?(values.reduce((a,b)=>a+b,0)/values.length).toFixed(2):'—';
  $('summary').textContent=`${rows.length} odgovora · zaposleni/uloge ${avg(scores)} / 5${!fe&&!fr?` · ceo tim ${avg(teamScores)} / 5`:''}`;
  $('results').replaceChildren(...rows.sort((a,b)=>b.submittedAt.localeCompare(a.submittedAt)).map(r=>{const item=node('div',null,{class:'result'});item.append(node('strong',`${r.clientName} · ${r.month}`),node('div',`Tim: ${r.team.map(x=>`${x.name} (${x.role})`).join(', ')}`));for(const q of r.questions){if(fr&&q.target!==fr)continue;if(fe&&q.targetEmployeeId!==fe)continue;const value=r.answers[q.id];if(value!==''&&value!=null)item.append(node('div',`${q.text} — ${value}`));}item.append(node('small',new Date(r.submittedAt).toLocaleString('sr-Latn-RS')));return item;}));
}
...
$('saveRoster').onclick=async()=>{const roles=[...$('roleRows').children].map(row=>({role:row.children[0].value,name:row.children[1].value.trim(),employeeId:row.children[2].value||null})).filter(x=>x.name);try{await api('assignment',{clientId:$('client').value,roles});await load();$('rosterEditor').hidden=true;message('Tim je sačuvan.');}catch(error){message(error.message);}};

...
function renderReviewedExamples(d,profiles=[]){
 ...
 const selected=[];const video=posts.find(p=>p.video),post=posts.find(p=>!p.video);if(video)selected.push(video);if(post)selected.push(post);if(selected.length<2)selected.push(...posts.filter(p=>!selected.includes(p)).slice(0,2-selected.length));
...
function contentBlueprint(d){const offer=d.offer||"glavna ponuda",city=d.location||"tvom mestu",program=/program|edukacij/i.test(`${d.offerType||""} ${d.business||""} ${offer}`),local=!program&&(d.salesScope==="Lokalno"||d.salesScope==="Online i lokalno"),caseTitle=local?`Primer klijenta iz mesta ${city}`:"Primer polaznika: od početnog problema do rezultata",trustTitle=local?`Zašto nam klijenti iz mesta ${city} veruju`:"Šta polaznik treba da zna pre prijave",trustBody=local?"Pokaži stvarni radni dan, lokaciju i razlog zbog kog lokalni klijenti dolaze baš kod vas.":"Objasni kome je program namenjen, kako izgleda podrška i šta osoba može realno da očekuje. Cenu i detalje možeš i dalje da sačuvaš za poziv.";return{
 reels:[[program?"Tri stvari koje roditelj treba da proveri pre izbora online programa":`Tri stvari koje treba proveriti pre izbora ponude „${offer}“`,`U prvoj rečenici jasno reci kome se obraćaš. Zatim pokaži jedan deo procesa i konkretan rezultat. Poziv na akciju neka vodi na ${d.purchasePath?.toLowerCase()||"razgovor"}.`],["Jedna greška koju publika često pravi","Izaberi problem koji se stvarno pojavljuje u pitanjima i reakcijama publike. Objasni posledicu i pokaži bolji pristup, bez agresivne prodaje."],[program?"Kome program nije namenjen":`Kome ponuda „${offer}“ nije namenjena`,`Iskren odgovor gradi poverenje. Objasni kome ne možeš da pomogneš i zašto.`],["Kako izgleda put od prvog razgovora do rezultata","Prikaži četiri kratka koraka i objasni zašto svaki od njih postoji."],[caseTitle,"Prikaži početnu situaciju, odluku, način rada i rezultat. Završetak treba prirodno da vodi ka razgovoru."]],
 stories:[["Jedan dan iza brenda — pet storija","Pokaži plan dana, deo pripreme, jedan detalj rada, odgovor na dilemu i pitanje publici."],["Problem, objašnjenje, pa razgovor","Objavi tri povezana storija, zatim anketu i jednostavan poziv da ti odgovore."],["Rezultat sa kontekstom","Pokaži rezultat, ali objasni početnu situaciju, šta je urađeno i šta je realno očekivati."],["Najčešće pitanje ove nedelje","Odgovori prirodno u videu, pa sledećim storijem ponudi razgovor ili prijavu."],[trustTitle,trustBody]],
 carousels:[[program?"Kako da znaš da li je ovaj program pravi izbor za tvoje dete":`Kako da prepoznaš da li ti je potrebna ponuda „${offer}“`,`Prvi slajd postavlja jasno pitanje. Srednji slajdovi pokazuju konkretne situacije u kojima se publika prepoznaje. Poslednji slajd vodi na razgovor.`],[program?"Kako izgleda rad u programu, bez otkrivanja svega":"Kako izgleda saradnja, korak po korak","Ukloni glavne neizvesnosti: kome je namenjeno, kako počinje, kakvu podršku klijent dobija i šta se očekuje od njega."],[program?"Šta roditelj treba da zna pre uvodnog razgovora":`Od čega zavisi vrednost ponude „${offer}“`,`Ne moraš javno da objaviš cenu. Objasni koje informacije osoba dobija na pozivu i kako da proceni da li je ponuda pravi izbor.`]],
 posts:[[`Studija slučaja sa konkretnim rezultatom`,`Fotografija ili kadar rezultata. U opisu: početni problem, odluka, proces, rezultat i sledeći korak.`],[`Lični stav stručnjaka`,`Objasni jednu stvar koju u industriji ne radite i zašto. Ovo diferencira brend.`],[`Ponuda kroz ishod`,`Ne nabrajaj karakteristike. Prikaži kako izgleda život ili biznis klijenta posle rešenog problema.`]]};}
...
function showProfileShell(data){
 ...
 const initial=username.replace(/^@/,"").charAt(0).toUpperCase();$("#scanAvatar").src=`data:image/svg+xml,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><rect width="100" height="100" rx="50" fill="#272727"/><text x="50" y="66" text-anchor="middle" font-family="Arial,sans-serif" font-size="52" font-weight="700" fill="#fff">${initial}</text></svg>`)}`;
}
...
$("#auditForm").onsubmit=async e=>{
 e.preventDefault();const d=Object.fromEntries(new FormData(e.target));localStorage.setItem("marketizoAudit",JSON.stringify(d));void captureLead(d);render(d);showProfileShell(d);show("analyzing");
 ...
 let publicProfiles=[];
 try{publicProfiles=await startProfilePrefetch(d);displayProfile(publicProfiles[0]);render(d,publicProfiles);inspectPost(publicProfiles[0]?.posts?.[0]);$("#analysisStatus").textContent="Sadržaj je učitan. Sada detaljno pregledamo objave i Reelove."}
 catch(error){clearInterval(waitingTimer);$("#feedGrid").innerHTML=`<div class="feed-unavailable"><strong>Profil trenutno nije dostupan</strong><p>${esc(error.message)}</p><small>Proveri da li je profil javan i da li je link tačno unet.</small></div>`;$("#analysisStatus").textContent="Proveri link profila i pokušaj ponovo.";addRetryAction();return}
